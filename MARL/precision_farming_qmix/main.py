...
row_length = config['field_length']
...
env = RowCultivationEnv(n_agents=n_agents, row_length=row_length, max_steps=max_steps)
obs_dim = env.observation_space.shape[1]  # Should be 1
action_dim = env.action_space.n  # Should be 4

state_dim = n_agents * obs_dim  # Flattened state dim
...
with open(log_file, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(["Episode", "AverageReward", "Hits", "FalsePositives", "Repeats"])  # header
...
for ep in range(episodes):
    ...
    # Track episode specific stats from environment for logging
    env_hits = 0
    env_false_positives = 0
    env_repeats = 0
    ...
    while not done:
        ...
        # Track custom stats from env attributes (hits, false positives, repeats)
        env_hits = env.hits
        env_false_positives = env.false_positives
        env_repeats = env.repeats

        # Also break if max_steps reached, but env.done covers that

    # Convert to numpy arrays for replay buffer
    ...
    # Logging average reward (per timestep per agent) and custom stats to CSV
    avg_reward = ep_reward / (n_agents)
    with open(log_file, 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([ep + 1, avg_reward, env_hits, env_false_positives, env_repeats])
    ...
    if len(replay_buffer.buffer) >= batch_size:
        ...
        s_batch = np.array([ep[0] for ep in padded_batch])    # (batch, ep_len, n_agents, obs_dim)
        a_batch = np.array([ep[1] for ep in padded_batch])    # (batch, ep_len, n_agents)
        r_batch = np.array([ep[2] for ep in padded_batch])    # (batch, ep_len, n_agents)
        ns_batch = np.array([ep[3] for ep in padded_batch])   # (batch, ep_len, n_agents, obs_dim)
        d_batch = np.array([ep[4] for ep in padded_batch])    # (batch, ep_len)

        for t in range(max_ep_len):
            obs_t = torch.tensor(s_batch[:, t], dtype=torch.float32).to(device)
            acts_t = torch.tensor(a_batch[:, t], dtype=torch.long).to(device)
            rews_t = torch.tensor(r_batch[:, t], dtype=torch.float32).to(device)
            next_obs_t = torch.tensor(ns_batch[:, t], dtype=torch.float32).to(device)
            done_t = torch.tensor(d_batch[:, t], dtype=torch.bool).to(device)
    ...
    if (ep + 1) % save_interval == 0:
        ...
        print(f"[Episode {ep + 1}] Checkpoint saved, epsilon={epsilon:.3f}")

    if (ep + 1) % 10 == 0:
        print(f"[Episode {ep + 1}] Epsilon: {epsilon:.3f} | AvgReward: {avg_reward:.4f} | Hits: {env_hits} | False Positives: {env_false_positives} | Repeats: {env_repeats}")
